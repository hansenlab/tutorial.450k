...
[^deprecated]: The `read.metharray`, `read.metharray.exp`, and `read.metharray.sheet` functions supersede the now-deprecated `read.450k`, `read.450k.exp`, and `read.450k.sheet` functions, respectively. The `read.metharray*` functions also generalise this functionality to the EPIC array.
...
The `bumphunter` function in minfi is a version of the bump hunting algorithm [@bumphunter] adapted to the 450k array, relying on the `bumphunter` function implemented in the eponymous package, `r Biocpkg("bumphunter")` 
...
__TODO:__ Do we want to include some/all of these and where? If so, include some discussion of plotting options

- `mdsPlot`
- `plotCpg`
- Using `r Biocpkg("Gviz")`


```{r, eval = FALSE, echo = FALSE, mdsPlot}
# NOTE: Quite different plots for different normalisation strategies, might be 
...
```{r, eval = FALSE, echo = FALSE, plotCpg}
...
sizes <- c(2, 2, 5, 2, 2, 3) # set up the relative sizes of the tracks
plotTracks(tracks, 
           from = minbase, 
           to = maxbase, 
           showTitle = TRUE, 
